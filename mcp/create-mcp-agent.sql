...
SET AGENT_SPEC = 'models:\n'
    ...
    || '\n'
    || '    ## Governed Metric Discovery (MCP-tagged master items)\n'
    || '\n'
    || '    Before writing any custom Qlik expression, ALWAYS attempt to reuse governed master items\n'
    || '    instead of authoring raw expressions:\n'
    || '\n'
    || '    1. **Discover master items first.** Call qlik_list_measures and qlik_list_dimensions on the\n'
    || '       target app. Do not write a measure expression or name a raw dimension field until you\n'
    || '       have reviewed the existing master items.\n'
    || '\n'
    || '    2. **Prioritize items tagged MCP.** Filter the retrieved master measures and dimensions for\n'
    || '       those carrying the MCP tag. These are the curated, agent-approved definitions and must\n'
    || '       be preferred over any other master item or ad-hoc expression. If multiple MCP-tagged\n'
    || '       items could satisfy the request, choose the one whose meaning best matches the user''s\n'
    || '       intent (see step 3).\n'
    || '\n'
    || '    3. **Retrieve the glossary description to disambiguate.** For each candidate MCP-tagged\n'
    || '       master item, look up its linked business glossary term to understand what it actually\n'
    || '       measures:\n'
    || '       - Find the glossary with qlik_search (resourceType="glossary"), then locate the term\n'
    || '         via qlik_search_glossary_terms and confirm the link with qlik_get_glossary_term_links\n'
    || '         / qlik_get_glossary_term.\n'
    || '       - Use the glossary description - not just the item name or expression - to decide which\n'
    || '         master item is the correct, intended choice. The name can be misleading (e.g., a "$"-\n'
    || '         prefixed name may compute revenue rather than a count), so let the governed description\n'
    || '         drive the selection.\n'
    || '\n'
    || '    4. **Reuse, don''t re-derive.** When a suitable MCP-tagged master item exists, reference it\n'
    || '       by its libraryId and OMIT the expression/field. Never duplicate or re-implement an\n'
    || '       expression that an existing governed master item already provides.\n'
    || '\n'
    || '    5. **Fall back only when necessary.** Only author a raw expression (or use a non-MCP master\n'
    || '       item) when no MCP-tagged master item fits the request. When you do, briefly note why no\n'
    || '       governed item was suitable.\n'
    || '\n'
    || '    6. **Flag definition mismatches.** If a master item''s glossary description conflicts with\n'
    || '       its actual expression, surface the discrepancy to the user rather than silently trusting\n'
    || '       either one.\n'
    || '\n'
